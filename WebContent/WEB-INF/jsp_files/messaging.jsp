						            		...
						            		<li class="list-group-item" style="background-color: #C0C0C0;" onclick="window.location.href='${pageContext.request.contextPath}/MessageHandler?action=getMessages&user1=${conversationItem.id.userId1}&user2=${conversationItem.id.userId2}'">
							            		<div class="row">
								            		<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
										              <img  class="img-circle profileImage" src="<c:out value="${conversationItem.photoURL}"/>">
										            </div>
										            <div class="col-xs-8 col-sm-8 col-md-8 col-lg-8">
										              <div class="d-flex w-100 justify-content-between">
										                    <h5 class="mb-1" style="text-align:center"><c:out value="${conversationItem.name}"/> <c:out value="${conversationItem.surname}"/></h5>
							                	...
							                	<p class="lastMsg"><c:out value="${conversationItem.messages[length-1].text}"/></p>
							            	</li>
						            	</c:when>
					            		<c:otherwise>
				            ...
				            <ul class="messageList col-xs-12 col-sm-12 col-md-12 col-lg-12">
								 ...
								 <c:forEach items="${conversation.messages}" var="message">
							 		...
							 		<c:if test="${messageUser == 'userId1'}">
							 			<li class="me messageListItem"><c:out value="${message.text}"/><p class="datePostedMe">${message.dateInterval}</p></li>
						    		</c:if>
						    		<c:if test="${messageUser == 'userId2'}">
							 			<li class="me messageListItem"><c:out value="${message.text}"/><p class="datePostedMe">${message.dateInterval}</p></li>
						    		</c:if>
							    	<c:if test="${messageUser == 'null'}">
							 			<li class="him messageListItem"><c:out value="${message.text}"/><p class="datePostedHim">${message.dateInterval}</p></li>
						    		</c:if>	
								 </c:forEach>
							</ul>
